...
- **一键恢复官方 AT 固件**：救砖页按清单自动出现按钮（清单含 `bw16-at` 条目即激活）。
  AT 三件套源自安信可官方文档站的 ComboAT 整片镜像（16Mbit v4.9.2，2MB/4MB 模块通吃），
  按分区布局切分（km0/km4/image2，已规避 FTL/校准区）；
  换新版 ComboAT 时下载后重新切分覆盖 `bw16/_at_firmware/flash/a/` 重跑构建即可
