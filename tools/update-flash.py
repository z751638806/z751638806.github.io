...
PATCHES = {
    "index.html": [
        ...
        # 一键救砖面板（免上传）：恢复目标二选一（出厂固件 / 官方 AT）+ 覆盖声明
        ("引导程序已内置，无需额外提供。\n        </p>",
         "引导程序已内置，无需额外提供。\n        </p>\n        <div class=\"alert alert-info\" style=\"margin:2px 0 10px;\">\n          <strong>⚡ 一键救砖（免上传）· 先选恢复目标</strong>\n          <div style=\"display:flex;flex-direction:column;gap:6px;margin:8px 0;\">\n            <label class=\"rescue-target\"><input type=\"radio\" name=\"rescue-target\" value=\"factory\" checked>\n              <span><b>出厂固件</b> — WiFi安全测试固件「2026」，恢复到你购买时的可用状态</span></label>\n            <label class=\"rescue-target\"><input type=\"radio\" name=\"rescue-target\" value=\"at\">\n              <span><b>官方 AT 固件</b> — 安信可原厂 ComboAT，干净的 AT 指令底座（二次开发用）</span></label>\n          </div>\n          <label class=\"advanced-row\" style=\"margin:8px 0;\">\n            <input type=\"checkbox\" id=\"oneclick-agree\">\n            我已了解：刷写将<b>擦除并覆盖</b>板上现有固件数据（不可撤销）\n          </label>\n          <button id=\"btn-oneclick-rescue\" class=\"btn btn-primary\" type=\"button\">⚡ 一键恢复所选固件</button>\n        </div>", 1),
    ...
    "js/rescue.js": [
        ...
        ("  $('btn-rescue-flash').addEventListener('click', onRescueClick);",
         "  $('btn-rescue-flash').addEventListener('click', onRescueClick);\n  $('btn-oneclick-rescue').addEventListener('click', async () => {\n    if (!$('oneclick-agree').checked) {\n      showResult('warn', '请先勾选「我已了解将覆盖板上固件」后再开始一键恢复。');\n      return;\n    }\n    setBusy(true);\n    const target = (document.querySelector('input[name=\"rescue-target\"]:checked') || {}).value || 'factory';\n    const source = target === 'at'\n      ? (state.manifests.firmware.find((f) => f.slug === 'bw16-at') || (() => { throw new Error('官方 AT 固件未内置'); })()).files\n      : state.manifests.sdkBackup;\n    try {\n      for (const name of Object.keys(SLOTS)) {\n        const meta = source?.[name];\n        if (!meta) throw new Error(`内置镜像缺失：${name}`);\n        markImage(name, await fetchBinary(meta.path), target === 'at' ? '官方 AT 固件' : '出厂固件');\n      }\n    } catch (e) {\n      setBusy(false);\n      showResult('err', `× 内置镜像加载失败：${e.message}`);\n      return;\n    }\n    setBusy(false);\n    await onRescueClick();\n  });", 1),
    ],
}
...
/* ---- 一键救砖：恢复目标单选卡片 ---- */
.rescue-target { display: flex; gap: 8px; align-items: flex-start; cursor: pointer; padding: 7px 10px; border: 1px solid var(--border); border-radius: 8px; background: var(--bg-panel-2); transition: border-color .2s; }
.rescue-target:hover { border-color: var(--accent); }
.rescue-target:has(input:checked) { border-color: var(--accent); box-shadow: 0 0 0 1px var(--accent); }
.rescue-target input { margin-top: 4px; }
.rescue-target b { color: var(--accent); }
/* ---- 页脚 ---- */
